...
>   summarize(mean_age_days=mean(age_dys))
> ```
> Or if we want to change `meta` itself: 
...
Using the `meta` data, use ggplot to create a box plot with continent on the x axis and life expectancy on the y axis. 
...
```{r}
inner_join(meta, raw_counts_long, by = 'libID')
```
...
```{r}
full_data <- inner_join(meta, raw_counts_long, by = 'libID')
```
