...
```{r}
#Attach data
attach("3_RNAseq_cleaning/dat_voom.RData")
#Rename data object to be shorter
dat <- dat.abund.norm.voom
```

# 2. Modeling in `limma`
## 2.1 Simple linear regression in `limma`
...
## 2.2 Paired sample design in `limma`
...
save(fit_kimma, file="4_linear_models/kimma_results.RData")
```
...
```{r}
load("4_linear_models/kimma_results.RData")
```
...
We can now look at metrics like AIC where we see that best fit still varies by gene (which is very common).

```{r}
full_join(fit_kimma$lm.fit, fit_kimma$lme.fit, 
