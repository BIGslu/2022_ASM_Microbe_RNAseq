...
```{r}
#Attach data
attach("/Users/heh39/Library/CloudStorage/Box-Box/CWRU/Talks/RNAseq 2022/dat_voom.RData")
...
```{r}
ttestRes<-t.test(gene ~ condition, data = modelDat)
ttestRes
```
...
```{r}
lmMod<-lm(gene ~ condition, data = modelDat)
summary(lmMod)
```
...
```{r}
lmModBig<-lm(gene ~ condition + age_dys + sex, data = modelDat)
summary(lmModBig)
```
...
```{r}
lmeMod<-lmer(gene ~ condition + (1 | ptID), data = modelDat)
summary(lmeMod)
anova(lmeMod)
```

```{r}
lmeModBig<-lmer(gene ~ condition + sex + age_dys + (1 | ptID), data = modelDat)
summary(lmeModBig)
```
