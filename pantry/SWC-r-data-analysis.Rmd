...
```{r, include=FALSE}
source("1_introR/chunk-options.R")
knitr_fig_path("04-")
```
...
We will start by importing the complete gapminder dataset that we used yesterday into our fresh new R session. Yesterday we did this using a "point-and-click" commands. Today let's type them into the console ourselves: `gapminder_data <- read_csv("1_introR/data/gapminder_data.csv")`
...
```{r ReloadGapminder}
gapminder_data <- read_csv("1_introR/data/gapminder_data.csv")
```
...
Let's say we would like to know what is the mean (average) life expectency in the dataset. R has a built in function function called `mean()` that will calculate this value for us. We can apply that function to our lifeExp column using the `summarize()` function. Here's what that looks like:
```{r AvgLifeExp}
...
> > gapminder_data_2007 <- read_csv("1_introR/data/gapminder_data.csv") %>%
...
read_csv("1_introR/data/co2-un-data.csv")
```
...
read_csv("1_introR/data/co2-un-data.csv", skip=1)
```
...
co2_emissions_dirty <- read_csv("1_introR/data/co2-un-data.csv", skip=2,
...
```{r Co2Recode}
co2_emissions <- read_csv("1_introR/data/co2-un-data.csv", skip=2,
...
```{r writeCSV}
write_csv(gapminder_co2, "1_introR/data/gapminder_co2.csv")
```
